...
public class SetupImpl extends MinimalEObjectImpl.Container implements Setup
{
   ...
   * @generated NOT
   */
  public EList<SetupTask> getSetupTasks(boolean filterRestrictions, boolean onlyInstall)
  {
    EList<SetupTask> setupTasks = new BasicEList<SetupTask>();

    Preferences preferences = getPreferences();
    getSetupTasks(filterRestrictions, onlyInstall, setupTasks, preferences);

    Branch branch = getBranch();
    getSetupTasks(filterRestrictions, onlyInstall, setupTasks, branch);

    Project project = branch.getProject();
    getSetupTasks(filterRestrictions, onlyInstall, setupTasks, project);

    EclipseVersion eclipseVersion = getEclipseVersion();
    getSetupTasks(filterRestrictions, onlyInstall, setupTasks, eclipseVersion);

    return setupTasks;
  }

  private void getSetupTasks(boolean filterRestrictions, boolean onlyInstall, EList<SetupTask> setupTasks,
      SetupTaskContainer container)
  {
    Branch branch = getBranch();
    Project project = branch.getProject();
    ...
    {
      if (!onlyInstall || setupTask instanceof InstallTask)
      {
